...
class Topic {
  ...
  /**
   * Subscribes to `Topic.topic`. All messages will be
    ...
    try {
      ...
      switch (request.type) {
        ...
        case Request.Type.SEND_MESSAGE:
          this.messageHandler({
            from: message.from,
            event: uint8arrayToString(request.event.name),
            message: {
  ...
  async send (event, message) {
    let _msg = {
      ...
      event: {name: uint8arrayFromString(event)},
      sendMessage: {
